...
class OpenIDRequestHandler(BaseHTTPRequestHandler):
    ...
    def getSession(self):
        ...
        if cookie_str:
            cookie_obj = cookies.SimpleCookie(cookie_str)
    ...
    def doVerify(self):
        ...
        try:
            ...
            fetch_error_string = 'Error in discovery: %s' % (
                ...
                html.escape(six.text_type(exc[0])))
    ...
    def renderSREG(self, sreg_data):
        if not sreg_data:
            ...
            #sreg_text = 
            self.wfile.write('<h2>Registration Data</h2>'
                '<table class="sreg">'
    ...
    def renderPAPE(self, pape_data):
        if not pape_data:
            ...
            for policy_uri in pape_data.auth_policies:
                self.wfile.write('<li><tt>%s</tt></li>' % (html.escape(policy_uri),))
...
'''
        self.wfile.write( (head_text % (title, title)).encode())
...
'''
        self.wfile.write( (footer_text % (quoteattr(self.buildURL('verify')), quoteattr(form_contents))).encode())
